...
class FeedForwardLayer():
    def __init__(self, input_shape=None, neurons=1, bias=None, weights=None, activation=None, is_bias=True):
        ...
        if self.input_shape is not None:
            self.weights = weights if weights is not None else np.random.randn(self.input_shape, neurons)
            self.parameters = self.input_shape * self.neurons + self.neurons if self.isbias else 0
        if is_bias:
            self.biases = bias if bias is not None else np.random.randn(neurons)
        else:
            self.biases = 0
        self.out = None
        self.input = None
    ...
    def set_n_input(self):
        self.weights = self.w if self.w is not None else np.random.normal(size=(self.input_shape, self.neurons))
